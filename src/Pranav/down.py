...
if __name__=="__main__":
    for j in range(1,7):
        ...
        if j==1:images = [cv2.resize(img,(1000,1000)) for img in images]
        else:images = [cv2.resize(img,(1000,1000)) for img in images]
        #plt.imshow(images[0])
        #plt.show()
        for i in range(len(images)):
            cv2.imwrite(f"Images/I{j}_down/image{i}.png",images[i])